%Delay = linspace(13,22,2*(22-13+1)-1);
%DD = length(Phase);
Delay = 18;
for TT = 1:length(Delay)

clearvars -except Delay TT
close all
clc
disp('Coming up with time estimate...')
%disp(['Phase ' num2str(Phase(TT))])
...
hbar = 0.658;
lighteV = 0; %eV
mu = 0*(XX<3).*(YY<3).*(ZZ<3); %eV
%0.05
xposition = -1.25; %Angstrom
carrierphase = 0; 
pulsedelay = Delay(TT); %fs
pulselength = 5;
Woffset = 5;
XX01 = -1.0557;
XX02 = .944;
...
TE = conservation;
DP = conservation;
DipoleExpectation = conservation;
...
for ll = 1:tNN
    ...
    if omegaLight ~=0 && (ll-1)*dt >= pulsedelay && (ll-1)*dt <= pulsedelay+pulselength
        Dipolet = (sin(2*pi/(2*pulselength)*((ll-1)*dt-pulsedelay))).^2.*sin(omegaLight*((ll-1)*dt-pulsedelay));
    %elseif omegaLight ~=0 && (ll-1)*dt >= pulsedelay && (ll-1)*dt <= pulsedelay+pulselength && Phase(TT) == 0.5
    %    Dipolet = (sin(2*pi/(2*pulselength)*((ll-1)*dt-pulsedelay))).^2.*cos(omegaLight*((ll-1)*dt-pulsedelay));
    %elseif omegaLight ~=0 && (ll-1)*dt >= pulsedelay && (ll-1)*dt <= pulsedelay+pulselength && Phase(TT) == 1
    %    Dipolet = -(sin(2*pi/(2*pulselength)*((ll-1)*dt-pulsedelay))).^2.*sin(omegaLight*((ll-1)*dt-pulsedelay));
    %elseif omegaLight == 0
    %    Dipolet = 1;
    else
    ...
    TE(ll) = PE(ll)+KE(ll);
    DipoleExpectation(ll) = sum(sum(sum(mu.*Awave1.*conj(Awave2))));
        
    XcenterA2(ll) = real(sum(sum(sum(Awave2.*XX.*conj(Awave2)))))/real(sum(sum(sum(Awave2.*conj(Awave2)))));
    XcenterA1(ll) = real(sum(sum(sum(Awave1.*XX.*conj(Awave1)))))/real(sum(sum(sum(Awave1.*conj(Awave1)))));
    XcenterD1(ll) = real(sum(sum(sum(Dwave1.*XX.*conj(Dwave1)))))/real(sum(sum(sum(Dwave1.*conj(Dwave1)))));
    XcenterD2(ll) = real(sum(sum(sum(Dwave2.*XX.*conj(Dwave2)))))/real(sum(sum(sum(Dwave2.*conj(Dwave2)))));
    %{
    if ll == 1
...
save(['C:\Chelsea\TDSE_Output\BigFix\Interference3LNoLightx1.25L0.1.mat'], '-v7.3')
%save(['C:\Chelsea\TDSE_Output\BigFix\Interference1eVSinSq5fs10LDelay' num2str(Delay(TT)) 'fs.mat'], '-v7.3')
